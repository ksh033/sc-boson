...
export default abstract class AbsSdk implements IJsSdk {
  ...
  constructor(opt: IJsSdkOption = { authUrl: '', sdkConfig: undefined }) {
    this.option = opt;
  }
  protected getSdkConfig(): IJsSdkConfig | null {
  ...
  /**
   * 支持：博思SDK 返回二维码页面并执行相应操作
   *
   * @param type 1 授权 2 刷新
   */
  startQrCodeActivity(type: any): Promise<IResult> {
    return new Promise((resolve, reject) => {
