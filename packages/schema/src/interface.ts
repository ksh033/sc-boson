...
export interface ButtonTypeProps extends ButtonProps {
  ...
  /**
   * 提交前处理
   * @description 返回true 表示继续 ，false 表示中断, values: 在表单中是表单数据 在页面调整和弹出是对应的参数
   */
  preHandle?: (values: any) => boolean;
...
export declare type ProColumn<RecordType=any> = (ProColumnGroupType<RecordType> | ProColumnType<RecordType>);

export declare type ProColumns<RecordType = unknown> = (ProColumnGroupType<RecordType> | ProColumnType<RecordType>)[];
...
export interface FormFilterProp {
  ...
  action?: string
  fieldsProp?:
    | Map<string, (item: FormItemProp, group: string) => FormItemProp | any>
    | any
}
...
export interface SearchFilterProp {
  ...
  action?: string
  fieldsProp?:
    | Record<string, (item: QueryConfigItem) => QueryConfigItem | any>
    | undefined
}
