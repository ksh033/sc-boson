...
import Header from '@ant-design/pro-layout/es/Header';
import type { AppMenuProps, MenuDataItem, MessageDescriptor, Route, RouterTypes, WithFalse } from '../typings';
...
const findAppCode = (pathname: string | undefined, appMenu?: AppMenuProps[]) => {
  const keys = pathname ? pathname.replace('//', '/').split('/') : [];
...
const renderSiderMenu = (
  ...
  const { layout, isMobile, openKeys, splitMenus, menuRender, apps, appMenu } = props;
  ...
  const siderMenuDom = renderSiderMenu(
    {
      ...
      onCollapse,
      isMobile,
      theme: (navTheme || 'dark').toLocaleLowerCase().includes('dark') ? 'dark' : '',
      collapsed,
      appSelected,
