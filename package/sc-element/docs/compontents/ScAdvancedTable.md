...
| columnsResizable | 是否开启列宽拖动功能（开启此功能还需要配置columns的width属性） | boolean | false |
| canRowMove | 是否开启行拖动功能（开启此功能还需要自己编写数据改变的逻辑） | boolean | false |
| onMoveRow | 需要开启canRowMove，当行拖动后触发，可获得拖动目标行的索引和放置行的索引，从而改变数据的排序 | (dragIndex, hoverIndex)=>void | null |
